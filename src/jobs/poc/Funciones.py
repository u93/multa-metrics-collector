...
def Statement_change(bbdd, parameter=None, horainicio=None, horafin=None, fechainicio=None, fechafin=None):
    ...
    if horainicio != None or horafin != None:
        bbdd = bbdd[(bbdd['tiempo'] > dt.datetime.strptime(horainicio, "%H:%M:%S").time()) &
    ...
    if fechainicio != None or fechafin != None:
        bbdd = bbdd[(bbdd['fecha'] > dt.datetime.strptime(fechainicio, "%Y-%m-%d").date()) &
    ...
    return print(bbdd[['serial_number', 'status', 'tiempo', 'cambio_estado', 'fecha']].head(40)),\
           print(bbdd[bbdd['cambio_estado']==1][['cambio_estado', 'status', 'tiempo', 'fecha']]), print(sum(bbdd['cambio_estado']))
